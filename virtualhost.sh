...
if [ "$action" == 'create' ]
	then
		...
		if ! [ -d $rootDir ]; then
			...
			if ! echo "<?php echo phpinfo(); ?>" > $rootDir/xphpinfoxxx.php
			then
				echo $"ERROR: Not able to write in file $rootDir/xphpinfoxxx.php. Please check permissions"
				exit;
			else
				echo $"Added content to $rootDir/xphpinfoxxx.php"
			fi
		fi
